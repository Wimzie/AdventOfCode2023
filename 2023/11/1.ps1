...
ForEach($position in $galaxyPositions) {
    ...
    if($rowExpand) {
        $correctedGalaxyPositions += $position.Replace("$row,", "$($row + $rowExpand.Count),")
        continue
    }
...
ForEach($position in $galaxyPositions) {
    ...
    if($columnExpand) {
        $correctedGalaxyPositions += $position.Replace(",$column", ",$($column + $columnExpand.Count)")
        continue
    }
    $correctedGalaxyPositions += $position
}
...
For($i = 0; $i -lt $galaxyPositions.Count -1; $i++) {
    for($j = $i + 1; $j -lt $galaxyPositions.Count; $j++) {
        ...
        $distance = [System.Math]::Abs(($x2 - $x1)) + [System.Math]::Abs(($y2 - $y1))
